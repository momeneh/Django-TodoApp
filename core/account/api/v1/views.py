...
class Registration(generics.GenericAPIView):
    ...
    def post(self, request, *arg ,**kwargs):
        ...
        html_template = render_to_string("emails/activation.tpl", {'token': token,'user':user})  
        html_inlined = transform(html_template)
        message = EmailMultiAlternatives('Activation Mail', html_inlined, 'from@example.cpm', [email])
        message.attach_alternative(html_inlined, "text/html")
        EmailThread(message).start() 
        return Response({'detail':'Registration succeeded ,activation link has been sent to the email'}, status=status.HTTP_200_OK)
...
class ActivationApiView(APIView):
   def get(self,request,token, *args, **kwargs):
        try:
           ...
           return Response( {'detail':'token has been expierd'},status=status.HTTP_400_BAD_REQUEST)
        except InvalidSignatureError:
           return Response( {'detail':'token is not valid'},status=status.HTTP_400_BAD_REQUEST)
        except DecodeError:
           return Response( {'detail':'token has a problem'},status=status.HTTP_400_BAD_REQUEST)
        
        user = User.objects.get(pk = user_id)
        if user.is_verified:
           return Response( {'detail':'your account has been verified'},status=status.HTTP_400_BAD_REQUEST)
...
class ActivationResendApiView(generics.GenericAPIView):
    ...
    def post(self,request, *args, **kwargs):
        ...
        html_template = render_to_string("emails/activation.tpl", {'token': token,'user':user})  
        html_inlined = transform(html_template)
        message = EmailMultiAlternatives('Activation Mail', html_inlined, 'from@example.cpm', [user.email])
        message.attach_alternative(html_inlined, "text/html")
        EmailThread(message).start() 
        return Response({'detail':'activation link has been sent to the email'}, status=status.HTTP_200_OK)
...
class ResetPasswordView(generics.GenericAPIView):
    ...
    def post(self,request, *args, **kwargs):
        ...
        token = get_tokens_for_user(user)
        reset_path = reverse('account:api-v1:reset-password-confirm', kwargs={'token': token})
        reset_url = request.build_absolute_uri(reset_path)

        html_template = render_to_string("emails/reset_password.tpl", {'user':user,'reset_url': reset_url,})  
        html_inlined = transform(html_template)
        message = EmailMultiAlternatives('Reset Password', html_inlined, 'from@example.cpm', [user.email])
        message.attach_alternative(html_inlined, "text/html")
        EmailThread(message).start() 
        return Response({'detail':'Reset Password link has been sent to the email'}, status=status.HTTP_200_OK)
