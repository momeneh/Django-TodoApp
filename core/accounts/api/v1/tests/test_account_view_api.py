...
class TestAccountsApi:
    ...
    def test_accounts_registration_validation_400(self, api_client):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:registration"), {"email": "ddd"}
        )
        assert response.status_code == 400

        response = api_client.post(
            reverse("accounts:api-v1:registration"),
            {"email": "ss@dd.com", "password": "1234", "password_confirmation": "1234"},
        )
        assert response.status_code == 400
    ...
    def test_accounts_change_password_400(self, api_client, common_user):
        ...
        response = api_client.put(
            reverse("accounts:api-v1:change-password"), {}
        )  # required
        assert response.status_code == 400
        ...
        )  # not match new passwords
        assert response.status_code == 400

        response = api_client.put(
            reverse("accounts:api-v1:change-password"),
            {"old_password": "1234!@#$", "password": "123", "confirm_password": "123"},
        )  # simple new password
        assert response.status_code == 400
    ...
    def test_accounts_reset_password_400(self, api_client):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:reset-password"), {"email": "wrong@email.com"}
        )
        assert response.status_code == 400

    def test_accounts_reset_password_200(self, api_client, common_user):
        response = api_client.post(
            reverse("accounts:api-v1:reset-password"), {"email": "admin@admin.com"}
        )
        assert response.status_code == 200
    ...
    def test_accounts_reset_password_confirm_400(self, api_client):
        ...
        response = api_client.get(
            reverse("accounts:api-v1:reset-password-confirm", kwargs={"token": "ddd"})
        )
        assert response.status_code == 400
    ...
    def test_accounts_token_login_400(self, api_client, common_user):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:login-token"), {}
        )  # required
        assert response.status_code == 400
    ...
    def test_accounts_jwt_create_400(self, api_client, common_user):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:jwt-create"), {}
        )  # required
        assert response.status_code == 400
    ...
    def test_accounts_jwt_refresh_401(self, api_client):
        response = api_client.post(
            reverse("accounts:api-v1:jwt-refresh"), {"refresh": "string"}
        )  # wrong refresh token
        assert response.status_code == 401
    ...
    def test_accounts_jwt_refresh_200(self, api_client, common_user):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:jwt-refresh"), {"refresh": refresh}
        )
        assert response.status_code == 200

    def test_accounts_jwt_verify_401(self, api_client):
        response = api_client.post(
            reverse("accounts:api-v1:jwt-verify"), {"token": "string"}
        )  # wrong  token
        assert response.status_code == 401
    ...
    def test_accounts_jwt_verify_200(self, api_client, common_user):
        ...
        response = api_client.post(
            reverse("accounts:api-v1:jwt-verify"), {"token": token}
        )
        assert response.status_code == 200
